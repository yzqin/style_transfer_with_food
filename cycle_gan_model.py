...
class CycleGANModel(BaseModel):
    ...
    def initialize(self, config):
        BaseModel.initialize(self, config)
        ...
        if self.isTrain:
            ...
            self.netD_A = networks.define_D(config.output_nc, config.ndf, config.netD,
                                            config.n_layers_D, config.norm, use_sigmoid, config.init_type, config.init_gain, self.gpu_ids)
            self.netD_B = networks.define_D(config.input_nc, config.ndf, config.netD,
                                            config.n_layers_D, config.norm, use_sigmoid, config.init_type, config.init_gain, self.gpu_ids)
    ...
    def set_input(self, input):
        ...
        print('Set loaded data to device:',self.device)
